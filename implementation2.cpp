...
int casee=0;
        ...
        if(food==-1)
        {
            //cout << "minus 15*(1-stamina/10): " << -15*(1-stamina/10) << endl;
            health=health-15*(1-stamina/10); //Health loss after loss of food now affected by stamina
            food=0;
        }
        if(food==-2)
        {
            //cout << "minus 30*(1-stamina/10): " << -16*(1-stamina/10) << endl;
            health=health-30*(1-stamina/10);
            food=0;
        }
        if(food==-3)
        {
            //cout << "minus 45*(1-stamina/10): " << -17*(1-stamina/10) << endl;
            health=health-45*(1-stamina/10);
            food=0;
        }
        if(health<0)
        {
    ...
    cout<<"Your score: "<<(money/day)<<endl; //Now desplaying score when winning
        }
...
health=hp;
stamina=st;
//cout << "hay look over here are we getting into the setcharacter meathod?" << endl;
//cout << "the characters stamina is: " << stamina << endl;
luck=lu;
intelligence=inte;
strength=str;
money=mon;
...
void character::loadVertexFile(char* fname)
{
    fstream infile;
    infile.open(fname);
    if (!infile.good())
...
void character::loadEdgeFile(char* fname)
{
    fstream infile;
    infile.open(fname);
    if (!infile.good())
